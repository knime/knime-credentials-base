...
/**
 * {@link Credential} implementation for situations where an authenticator node
 * (or backend) holds some credentials that can be used to dynamically acquire
 * access tokens for various scopes of access. This class should only be used by
 * authenticator nodes. The token fetching logic must be encapsulated in the
 * provided token fetcher function.
 *
 * <p>
 * Code that wants to consume the resulting access token should use the
 * {@link AccessTokenWithScopesAccessor} interface.
 * </p>
 *
